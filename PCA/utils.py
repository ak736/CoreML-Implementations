...
def find_embedding(word, words, E):
    ...
    # Find the index/position of 'word' in 'words'
    word_idx = np.where(np.array(words) == word)[0][0]

    # Return the row of 'E' at this index
    emb = E[word_idx]
...
def find_most_sim_word(word, words, E):
    ...
    # Get the word embedding of 'word'
    word_embedding = find_embedding(word, words, E)

    # Find the index/position of 'word' in 'words'
    word_idx = np.where(np.array(words) == word)[0][0]

    # Compute the similarity (dot product) of this embedding with every embedding
    similarities = np.dot(E, word_embedding)

    # Create a mask to exclude the input word
    mask = np.ones(len(words), dtype=bool)
    mask[word_idx] = False

    # Find the index of the row of 'E' with the largest similarity (excluding the input word)
    most_similar_idx = np.argmax(similarities * mask)

    # Find the word corresponding to that index
    most_sim_word = words[most_similar_idx]
...
def find_info_ev(v, words, k=20):
    ...
    # Get indices that would sort the entries of 'v' in decreasing order of absolute value
    ...
    info = [words[idx] for idx in top_k_indices]
...
def get_projections(word_seq, words, E, w):
    ...
    # Normalize w to ensure we get proper projections
    w_normalized = w / np.linalg.norm(w)

    # Initialize the sequence of projections
    proj_seq = []

    # For each word in word_seq, get its embedding and find projection onto w
    for word in word_seq:
        # Get the word embedding
        word_embedding = find_embedding(word, words, E)

        # Find the projection (dot product with normalized w)
        projection = np.dot(word_embedding, w_normalized)
...
def comparative_projections(word_seq, words, E, wd1, wd2):
    ...
    # Find embeddings for wd1 and wd2
    w1 = find_embedding(wd1, words, E)
    w2 = find_embedding(wd2, words, E)

    # Get the difference vector and normalize it
    w = w1 - w2
    w_normalized = w / np.linalg.norm(w)

    # Get projections of the words in word_seq onto this normalized vector
...
def find_most_sim_word_w(w, words, E, ids):
    ...
    # Compute the similarity (dot product) of 'w' with every embedding
    similarities = np.dot(E, w)

    # Create a mask to exclude the words with indices in 'ids'
    mask = np.ones(len(words), dtype=bool)
    mask[ids] = False

    # Find the index with the largest similarity (excluding the indices in 'ids')
    most_similar_idx = np.argmax(similarities * mask)
...
def find_analog(wd1, wd2, wd3, words, E):
    ...
    # Find the word embeddings of wd1, wd2 and wd3
    w1 = find_embedding(wd1, words, E)
    w2 = find_embedding(wd2, words, E)
    w3 = find_embedding(wd3, words, E)

    # Get vector w = w2 - w1 + w3, and normalize it
    w = w2 - w1 + w3
    w_normalized = w / np.linalg.norm(w)

    # Find the indices/positions of wd1, wd2, wd3 in 'words'
    idx1 = np.where(np.array(words) == wd1)[0][0]
    idx2 = np.where(np.array(words) == wd2)[0][0]
    idx3 = np.where(np.array(words) == wd3)[0][0]

    # Find wd4, excluding the given three words from the search
    wd4 = find_most_sim_word_w(w_normalized, words, E, [idx1, idx2, idx3])
...
def similarity(wd1, wd2, words, E):
    ...
    # Find the word embeddings of wd1 and wd2
    w1 = find_embedding(wd1, words, E)
    w2 = find_embedding(wd2, words, E)

    # Calculate cosine similarity (dot product of normalized vectors)
    # Since E is already normalized, we can directly use the dot product
