...
class ReplayBuffer:
    ...
    def sample(self, batch_size: int) -> BatchTransition:
        ...
        """
        ### YOUR CODE HERE ###
        # Create an empty BatchTransition
        empty_batch = BatchTransition(
            states=np.array([], dtype=int),
            actions=np.array([], dtype=int),
            rewards=np.array([], dtype=float),
            next_states=np.array([], dtype=int)
        )

        # Return empty batch if buffer is empty
        if self.size == 0:
            return empty_batch

        # If batch_size <= 0, return empty batch
        if batch_size <= 0:
            return empty_batch

        # If batch_size >= self.size, return all transitions
        if batch_size >= self.size:
            return BatchTransition.from_list(list(self.buffer))

        # Otherwise, sample without replacement
        sampled_transitions = random.sample(list(self.buffer), batch_size)
