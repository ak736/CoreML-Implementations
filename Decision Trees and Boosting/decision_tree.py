...
def train_model(X_train, t_train, max_depth=10, min_samples_leaf=10, criterion='gini',
    ...
    # AdaBoost with Decision Tree: TODO (call AdaBoost from boosting.py with appropriate parameters)
...
def get_acc(X_tr, t_tr, X_te, t_te, par_val, par_name, model_type):
    ...
    for i in par_val:
        ...
        if par_name == 'max_depth':
            model = train_model(X_tr, t_tr, model_type=model_type,
                                max_depth=i, min_samples_leaf=1)

        elif par_name == 'min_samples_leaf':
            model = train_model(X_tr, t_tr, model_type=model_type,
                                min_samples_leaf=i, max_depth=10)

        elif par_name == 'n_estimators':
            model = train_model(
                X_tr, t_tr, model_type=model_type, n_estimators=i)

        elif par_name == 'max_features':
            model = train_model(
                X_tr, t_tr, model_type=model_type, max_features=i)

        elif par_name == 'max_samples':
            model = train_model(
                X_tr, t_tr, model_type=model_type, max_samples=i)
...
def check_behavior(X_tr, X_te, y_tr, y_te, vals, name, model_type, ylim=0.75):
    ...
    acc_tr, acc_te = get_acc(X_tr, y_tr, X_te, y_te,
                             par_val=vals, par_name=name, model_type=model_type)
